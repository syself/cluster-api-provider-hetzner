...
type HetznerSecretRef struct {
	...
	// "hcloud". The Syself ccm fork defaults to "hetzner". To facilitate migration to the upstream
	// hcloud-ccm, the controller creates two secrets if this name is not "hcloud": one named
	// "hcloud" and one named after this setting. The secret will be created in the namespace
	// "kube-system" of the workload-cluster. We recommend to use the value "hcloud", because this
	// is the default of upstream hcloud-ccm. Set `spec.skipCreatingHetznerSecretInWorkloadCluster`,
...
type HetznerSecretKeyRef struct {
	...
	// to the upstream hcloud-ccm, an additional key gets created only in the workload-cluster
	// secret named "hcloud": if this field is not "token", then the same token value gets copied
	// to the key "token" in that secret. Other workload-cluster secrets only get the configured key.
	//
