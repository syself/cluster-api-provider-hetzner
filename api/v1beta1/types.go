...
type HetznerSecretKeyRef struct {
	...
	// secret remains untouched. CAPH only adds the compatibility keys "token" and "hcloud"
	// inside the workload-cluster secret named "hcloud", unless one of them already matches this
	// field. If the configured secret name is already "hcloud", the same token value is stored
	// under all required keys in that single secret. We recommend to use "token".
	//
