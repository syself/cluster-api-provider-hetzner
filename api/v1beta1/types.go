...
type HetznerSecretRef struct {
	...
	// "hcloud". The Syself ccm fork defaults to "hetzner". To stay compatible with upstream
	// hcloud-ccm, if this name is not "hcloud", the controller creates two secrets: one named
	// "hcloud" and one named after the value of this setting. The secret will be created in the
