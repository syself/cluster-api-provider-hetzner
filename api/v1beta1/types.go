...
type HetznerSecretRef struct {
	...
	// ccm fork defaults to "hetzner". To facilitate migration to the upstream hcloud-ccm, the
	// configured workload-cluster secret remains untouched. If this field is not "hcloud", CAPH
	// also creates an additional workload-cluster secret named "hcloud" in namespace "kube-system"
	// with the key names expected by upstream hcloud-ccm. We recommend to use the value "hcloud".
	// Set `spec.skipCreatingHetznerSecretInWorkloadCluster` if you don't want workload-cluster
