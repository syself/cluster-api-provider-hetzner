...
type LoadBalancerSpec struct {
	...
	//
	// Every field can be changed later and CAPH applies the change, with the timing below
	// for the first switch away from the TCP check. Removing HealthCheck is the exception:
	// CAPH then stops managing the check, and the load balancer keeps the last check that
	// was applied instead of going back to the TCP check.
	//
	// On an existing cluster, the switch from the TCP check to an http or https check waits
	...
	// control-plane infra machine template. Until then the TCP check stays in place, so the
	// switch never marks a backend that does not yet answer the path unhealthy. A new
	// cluster gets the configured check from the start. This works like the proxy-protocol
