...
def patch_args_with_extra_args(namespace, name, extra_args):
    args_str = str(
        local(
            "kubectl get deployments {} -n {} -o jsonpath='{{.spec.template.spec.containers[0].args}}'".format(
                name, namespace
            )
        )
    )
    ...
    if args_to_add:
        ...
        local(
            "kubectl patch deployment {} -n {} --type json -p='{}'".format(
...
def caph():
    ...
    build_cmd = (
        "{build_env} go build -ldflags '{ldflags}' -o .tiltbuild/manager".format(
            build_env=build_env,
            ldflags=ldflags,
        )
    )
...
def waitforsystem():
    local(
        "kubectl wait --for=condition=ready --timeout=300s pod --all -n capi-kubeadm-bootstrap-system"
    )
    local(
        "kubectl wait --for=condition=ready --timeout=300s pod --all -n capi-kubeadm-control-plane-system"
    )
...
def deploy_observability():
    k8s_yaml(
        blob(
            str(
                local(
                    "{} build {}".format(kustomize_cmd, "./hack/observability/"),
                    quiet=True,
                )
            )
        )
    ...
    k8s_resource(
        workload="promtail",
        extra_pod_selectors=[{"app": "promtail"}],
        labels=["observability"],
    )
    k8s_resource(
        workload="loki", extra_pod_selectors=[{"app": "loki"}], labels=["observability"]
    )
    k8s_resource(
        workload="grafana",
        port_forwards="3000",
        extra_pod_selectors=[{"app": "grafana"}],
        labels=["observability"],
    )
...
cmd_button(
    ...
    argv=[
        "make",
        "create-workload-cluster-hetzner-baremetal-control-plane-remediation",
    ],
    location=location.NAV,
    icon_name="dvr",
