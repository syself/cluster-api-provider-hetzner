...
func TestHealthCheckDiffers(t *testing.T) {
	...
	tests := []struct {
		...
		desired  *infrav2.LoadBalancerHealthCheckSpec
		want     bool
	}{
		{
		...
		},
		{
			...
			desired:  &infrav2.LoadBalancerHealthCheckSpec{Protocol: "https", Path: ptr.To("/readyz")},
			want:     false,
		},
		{
		...
		},
		{
			...
			observed: hcloud.LoadBalancerServiceHealthCheck{Protocol: "tcp", Port: servicePort},
			desired:  &infrav2.LoadBalancerHealthCheckSpec{Protocol: "tcp", Port: ptr.To(8443)},
			want:     true,
		},
		{
			...
			desired:  &infrav2.LoadBalancerHealthCheckSpec{Protocol: "tcp", IntervalSeconds: ptr.To(5)},
			want:     true,
		},
		{
			...
			desired:  &infrav2.LoadBalancerHealthCheckSpec{Protocol: "https", Path: ptr.To("/readyz"), StatusCodes: []string{"200"}},
			want:     true,
		},
	}
...
func fullHealthCheckSpec() *infrav2.LoadBalancerHealthCheckSpec {
...
func TestHealthCheckAddOptsTCPHasNoHTTP(t *testing.T) {
	got := healthCheckAddOpts(&infrav2.LoadBalancerHealthCheckSpec{Protocol: "tcp"}, 6443)
	if got == nil {
		t.Fatal("healthCheckAddOpts() = nil, want opts")
	}
	if got.HTTP != nil {
...
func TestHealthCheckCreateOptsTCPHasNoHTTP(t *testing.T) {
	got := healthCheckCreateOpts(&infrav2.LoadBalancerHealthCheckSpec{Protocol: "tcp"}, 6443)
	if got == nil {
		t.Fatal("healthCheckCreateOpts() = nil, want opts")
	}
	if got.HTTP != nil {
...
func TestHealthCheckMigratesToHTTP(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			desired:  &infrav2.LoadBalancerHealthCheckSpec{Protocol: "http", Path: ptr.To("/readyz")},
			want:     true,
		},
		{
		...
		},
		{
			...
			observed: hcloud.LoadBalancerServiceHealthCheck{Protocol: "tcp"},
			desired:  &infrav2.LoadBalancerHealthCheckSpec{Protocol: "tcp"},
			want:     false,
		},
		{
