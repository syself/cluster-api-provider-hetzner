...
func (s *Service) actionPreparing(ctx context.Context) actionResult {
	...
	if err != nil {
		...
		if models.IsError(err, models.ErrorCodeServerNotFound) {
			...
			s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStatePermanentError, msg)
			return actionStop{}
		}
	...
	if s.scope.SSHAfterInstallImageEnabled() {
		...
		if trimLineBreak(out.StdOut) != "" {
			...
			s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStateSSHRebootTriggered, fmt.Sprintf("Phase %s, reboot via ssh: %s",
	...
	s.scope.HetznerBareMetalHost.SetOperationalState(errorType, msg)
	return actionComplete{} // next: Registering
}
	...
	if !foundSSHKey {
		...
		if err != nil {
			...
			if models.IsError(err, models.ErrorCodeKeyAlreadyExists) {
				...
				s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStatePreparationError, msg)
...
func (s *Service) handleIncompleteBoot(ctx context.Context, isRebootIntoRescue, isTimeout, isConnectionRefused bool) (failed bool, err error) {
	...
	if isConnectionRefused {
		if s.scope.HetznerBareMetalHost.Status.OperationalState == infrav2.OperationalStateConnectionError {
			...
			s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStateConnectionError, "ssh gave connection error")
		}
		return false, nil
	}
	...
	if s.scope.HetznerBareMetalHost.Status.OperationalState == infrav2.OperationalStateConnectionError {
		s.scope.HetznerBareMetalHost.ClearOperationalState()
	...
	var emptyErrorType infrav2.OperationalState
	switch s.scope.HetznerBareMetalHost.Status.OperationalState {
	case emptyErrorType:
		if isTimeout {
			...
			s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStateSSHRebootTriggered, "ssh timeout error - server has not restarted yet")
			return false, nil
		}
	...
	case infrav2.OperationalStateSSHRebootTriggered:
		return false, s.handleErrorTypeSSHRebootFailed(ctx, isTimeout, isRebootIntoRescue)

	case infrav2.OperationalStateSoftwareRebootTriggered:
		return false, s.handleErrorTypeSoftwareRebootFailed(ctx, isTimeout, isRebootIntoRescue)

	case infrav2.OperationalStateHardwareRebootTriggered:
		return s.handleErrorTypeHardwareRebootFailed(ctx, isTimeout, isRebootIntoRescue)
	}

	return false, fmt.Errorf("%w: %s", errUnexpectedErrorType, s.scope.HetznerBareMetalHost.Status.OperationalState)
}
...
func rebootAndErrorTypeAfterTimeout(host *infrav2.HetznerBareMetalHost) (infrav2.RebootType, infrav2.OperationalState) {
	var rebootType infrav2.RebootType
	var errorType infrav2.OperationalState
	switch {
	case host.HasSoftwareReboot():
		rebootType = infrav2.RebootTypeSoftware
		errorType = infrav2.OperationalStateSoftwareRebootTriggered
	case host.HasHardwareReboot():
		rebootType = infrav2.RebootTypeHardware
		errorType = infrav2.OperationalStateHardwareRebootTriggered
	default:
...
func (s *Service) handleErrorTypeSoftwareRebootFailed(ctx context.Context, isSSHTimeoutError, wantsRescue bool) error {
	...
	if !isSSHTimeoutError || hasTimedOut(s.scope.HetznerBareMetalHost.Status.RebootTriggeredAt, softwareResetTimeout) {
		...
		// we immediately set an error message in the host status to track the reboot we just performed
		s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStateHardwareRebootTriggered, msg)
	}
...
func (s *Service) actionRegistering(ctx context.Context) actionResult {
	...
	if hostName != rescue {
		...
		if failed {
			s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStatePermanentError, err.Error())
			return actionStop{}
		}
		if err != nil {
	...
	if s.scope.HetznerBareMetalHost.Spec.RootDeviceHints == nil {
		...
		s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStateRegistrationError, infrav2.ErrorMessageMissingRootDeviceHints)
	...
	if errMsg != "" {
		...
		s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStateRegistrationError, errMsg)
	...
	if err := validateRootDeviceWwnsAreSubsetOfExistingWwns(s.scope.HetznerBareMetalHost.Spec.RootDeviceHints,
		...
		s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStateRegistrationError, err.Error())
		// The user has to fix the root device hints. Check again in five minutes.
		return actionContinue{delay: 5 * time.Minute}
	}
	...
	if msg != "" {
		...
		s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStateFatalError, msg)
		return actionStop{}
	}
...
func (s *Service) actionPreProvisioning(ctx context.Context) actionResult {
	...
	if exitStatus != 0 {
		...
		s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStatePermanentError, output)
		return actionStop{}
	}
...
func (s *Service) actionImageInstallingImageURLCommand(ctx context.Context, sshClient sshclient.Client) actionResult {
	...
	if duration > 7*time.Minute {
		...
		host.SetOperationalState(infrav2.OperationalStateFatalError, msg)
		return actionStop{}
	}
	...
	case sshclient.ImageURLCommandStateNotStarted:
		...
		if exitStatus != 0 {
			...
			s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStateProvisioningError, msg)
...
func (s *Service) createAutoSetupInput(ctx context.Context, sshClient sshclient.Client) (autoSetupInput, actionResult) {
	...
	if errorMessage != "" {
		...
		s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStateProvisioningError, errorMessage)
		// Mid-install error. Retry in a minute.
...
func (s *Service) checkCloudInitStatus(ctx context.Context, sshClient sshclient.Client) (actionResult, string) {
	...
	case strings.Contains(stdOut, "status: disabled"):
		...
		createSSHRebootEvent(ctx, s.scope.HetznerBareMetalHost, msg)
		s.scope.HetznerBareMetalHost.Status.RebootTriggeredAt = metav1.Now()
		s.scope.HetznerBareMetalHost.SetOperationalState(infrav2.OperationalStateSSHRebootTriggered, "ssh reboot just triggered")
...
func (s *Service) actionDeprovisioning(ctx context.Context) actionResult {
	...
	if s.scope.HetznerBareMetalHost.Status.OperationalState != infrav2.OperationalStatePermanentError {
		s.scope.HetznerBareMetalHost.ClearOperationalState()
	}
...
func (s *Service) cleanupHostAfterDeprovisioning() {
	...
	// The permanent error details remain on the ActionCompleted condition.
...
func (s *Service) hasJustRebooted() bool {
	return (s.scope.HetznerBareMetalHost.Status.OperationalState == infrav2.OperationalStateSSHRebootTriggered ||
		s.scope.HetznerBareMetalHost.Status.OperationalState == infrav2.OperationalStateSoftwareRebootTriggered ||
		s.scope.HetznerBareMetalHost.Status.OperationalState == infrav2.OperationalStateHardwareRebootTriggered) &&
