...
var _ = Describe("SetOperationalState and ClearOperationalState", func() {
	type testCaseSetError struct {
		errorType      infrav2.OperationalState
		errorMessage   string
		expectedReason string
	}

	DescribeTable("SetOperationalState sets the error type, and surfaces genuine failures on the ActionCompleted condition",
		func(tc testCaseSetError) {
			host := helpers.BareMetalHost("test-host", "default")

			host.SetOperationalState(tc.errorType, tc.errorMessage)

			Expect(host.Status.OperationalState).To(Equal(tc.errorType))
			c := conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition)
			Expect(c).ToNot(BeNil())
			Expect(c.Status).To(Equal(metav1.ConditionFalse))
			Expect(c.Reason).To(Equal(tc.expectedReason))
			Expect(c.Message).To(Equal(tc.errorMessage))
		},
		Entry("registration error", testCaseSetError{
			errorType:      infrav2.OperationalStateRegistrationError,
			errorMessage:   "registration failed",
			expectedReason: infrav2.HetznerBareMetalHostRegistrationErrorReason,
		}),
		Entry("preparation error", testCaseSetError{
			errorType:      infrav2.OperationalStatePreparationError,
			errorMessage:   "preparation failed",
			expectedReason: infrav2.HetznerBareMetalHostPreparationErrorReason,
		}),
		Entry("provisioning error", testCaseSetError{
			errorType:      infrav2.OperationalStateProvisioningError,
			errorMessage:   "provisioning failed",
			expectedReason: infrav2.HetznerBareMetalHostProvisioningErrorReason,
		}),
		Entry("fatal error", testCaseSetError{
			errorType:      infrav2.OperationalStateFatalError,
			errorMessage:   "fatal failure",
			expectedReason: infrav2.HetznerBareMetalHostFatalErrorReason,
		}),
	)

	// The reboot-triggered and transient connection error types are operational states. They are
	// surfaced on the ActionCompleted condition with their own reason and status True, because they are
	// not failures (genuine failures report status False).
	DescribeTable("SetOperationalState surfaces operational states on the condition with status True and a matching reason",
		func(errorType infrav2.OperationalState, expectedReason string) {
			host := helpers.BareMetalHost("test-host", "default")

			host.SetOperationalState(errorType, "operational state")

			Expect(host.Status.OperationalState).To(Equal(errorType))
			c := conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition)
			Expect(c).ToNot(BeNil())
			Expect(c.Status).To(Equal(metav1.ConditionTrue))
			Expect(c.Reason).To(Equal(expectedReason))
			Expect(c.Message).To(Equal("operational state"))
		},
		Entry("ssh reboot triggered", infrav2.OperationalStateSSHRebootTriggered, infrav2.HetznerBareMetalHostSSHRebootTriggeredReason),
		Entry("software reboot triggered", infrav2.OperationalStateSoftwareRebootTriggered, infrav2.HetznerBareMetalHostSoftwareRebootTriggeredReason),
		Entry("hardware reboot triggered", infrav2.OperationalStateHardwareRebootTriggered, infrav2.HetznerBareMetalHostHardwareRebootTriggeredReason),
		Entry("connection error", infrav2.OperationalStateConnectionError, infrav2.HetznerBareMetalHostSSHConnectionRefusedReason),
	)

	It("reports an unrecognized state as True with the UnknownError reason", func() {
		host := helpers.BareMetalHost("test-host", "default")

		host.SetOperationalState(infrav2.OperationalState("some unknown state"), "unexpected")

		c := conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition)
		Expect(c).ToNot(BeNil())
		Expect(c.Status).To(Equal(metav1.ConditionTrue))
		Expect(c.Reason).To(Equal(infrav2.HetznerBareMetalHostUnknownErrorReason))
	})

	It("updates the condition when moving from a failure to an operational state", func() {
		host := helpers.BareMetalHost(
			"test-host",
			"default",
			helpers.WithError(infrav2.OperationalStateProvisioningError, "provisioning failed"),
		)
		Expect(conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition).Reason).To(Equal(infrav2.HetznerBareMetalHostProvisioningErrorReason))

		host.SetOperationalState(infrav2.OperationalStateSSHRebootTriggered, "reboot via ssh")

		Expect(host.Status.OperationalState).To(Equal(infrav2.OperationalStateSSHRebootTriggered))
		c := conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition)
		Expect(c).ToNot(BeNil())
		Expect(c.Reason).To(Equal(infrav2.HetznerBareMetalHostSSHRebootTriggeredReason))
		Expect(c.Message).To(Equal("reboot via ssh"))
	})
	...
	It("overwrites an existing error", func() {
		host := helpers.BareMetalHost(
			"test-host",
			"default",
			helpers.WithError(infrav2.OperationalStatePreparationError, "first message"),
		)

		host.SetOperationalState(infrav2.OperationalStateRegistrationError, "new message")

		Expect(host.Status.OperationalState).To(Equal(infrav2.OperationalStateRegistrationError))
		c := conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition)
		Expect(c).ToNot(BeNil())
		Expect(c.Reason).To(Equal(infrav2.HetznerBareMetalHostRegistrationErrorReason))
		Expect(c.Message).To(Equal("new message"))
	})

	It("sets the permanent error annotation for permanent errors", func() {
		host := helpers.BareMetalHost("test-host", "default")

		host.SetOperationalState(infrav2.OperationalStatePermanentError, "permanent failure")

		Expect(host.Status.OperationalState).To(Equal(infrav2.OperationalStatePermanentError))
		Expect(host.Annotations).To(HaveKey(infrav2.PermanentErrorAnnotation))
		c := conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition)
		Expect(c).ToNot(BeNil())
		Expect(c.Reason).To(Equal(infrav2.HetznerBareMetalHostPermanentErrorReason))
	})

	It("ClearOperationalState removes the error type and the ActionCompleted condition", func() {
		host := helpers.BareMetalHost(
			"test-host",
			"default",
			helpers.WithError(infrav2.OperationalStatePreparationError, "first message"),
		...
		host.ClearOperationalState()

		Expect(host.Status.OperationalState).To(Equal(infrav2.OperationalState("")))
...
var _ = Describe("actionImageInstalling (image-url-command)", func() {
	...
	// newBaseHost returns the host and the install image of the consuming machine. The install
	// image has to be set on the HetznerBareMetalMachine of the scope after newTestService.
	newBaseHost := func() (*infrav2.HetznerBareMetalHost, infrav1.InstallImage) {
		...
		// Install image with custom image-url-command mode
		...
		return host, installImage
	}

	It("returns continue when command is running", func() {
		host, installImage := newBaseHost()
		...
		svc.scope.HetznerBareMetalMachine.Spec.InstallImage = installImage

		res := svc.actionImageInstalling(ctx)
		Expect(res).To(BeAssignableToTypeOf(actionContinue{}))
		// TODO: Remove the duplicated checks of deprecated v1beta1 conditions once all resources are native v1beta2.
	...
	It("reboots and completes when command finished successfully", func() {
		...
		Expect(host.Status.OperationalState).To(BeEmpty())
		Expect(conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition)).To(BeNil())
		c := conditions.Get(host, infrav2.HetznerBareMetalHostProvisionSucceededCondition)
		Expect(c.Message).To(Equal(`host (test-host) is still provisioning - state "image-installing"`))
		deprecatedC := deprecatedv1beta1conditions.Get(host, infrav2.ProvisionSucceededV1Beta1Condition)
	...
	It("stops when command failed", func() {
		...
		Expect(host.Status.OperationalState).To(Equal(infrav2.OperationalStateFatalError))
		c := conditions.Get(host, infrav2.HetznerBareMetalHostProvisionSucceededCondition)
		Expect(c.Message).To(ContainSubstring("image-url-command failed"))
		deprecatedC := deprecatedv1beta1conditions.Get(host, infrav2.ProvisionSucceededV1Beta1Condition)
		Expect(deprecatedC.Message).To(ContainSubstring("image-url-command failed"))
	})
	...
	It("starts the command on NotStarted and continues", func() {
		...
		svc := newTestService(host, nil, bmmock.NewSSHFactory(sshMock, sshMock, sshMock), nil, helpers.GetDefaultSSHSecret(rescueSSHKeyName, "default"))
		svc.scope.HetznerBareMetalMachine.Spec.InstallImage = installImage
		commandPath := filepath.Join(baremetalImageURLCommandDir, installImage.ImageURLCommand)
		sshMock.On("StartImageURLCommand", mock.Anything, commandPath, installImage.Image.URL, mock.Anything, svc.scope.Hostname(), []string{"nvme1n1"}).Return(0, "", nil)
		...
		res := svc.actionImageInstalling(ctx)
		Expect(res).To(BeAssignableToTypeOf(actionContinue{}))
		Expect(sshMock.AssertCalled(GinkgoT(), "StartImageURLCommand", mock.Anything, commandPath, installImage.Image.URL, mock.Anything, svc.scope.Hostname(), []string{"nvme1n1"})).To(BeTrue())
		c := conditions.Get(host, infrav2.HetznerBareMetalHostProvisionSucceededCondition)
	...
	It("records failure when StartImageURLCommand returns non-zero exit", func() {
		...
		svc.scope.HetznerBareMetalMachine.Spec.InstallImage = installImage
		commandPath := filepath.Join(baremetalImageURLCommandDir, installImage.ImageURLCommand)
		sshMock.On("StartImageURLCommand", mock.Anything, commandPath, installImage.Image.URL, mock.Anything, svc.scope.Hostname(), []string{"nvme1n1"}).Return(7, "boom", nil)
		...
		Expect(host.Status.OperationalState).To(Equal(infrav2.OperationalStateProvisioningError))
		c := conditions.Get(host, infrav2.HetznerBareMetalHostProvisionSucceededCondition)
	...
	It("times out after 7 minutes", func() {
		host, installImage := newBaseHost()
		host.Status.RebootTriggeredAt = metav1.NewTime(time.Now().Add(-8 * time.Minute))
...
var _ = Describe("handleIncompleteBoot", func() {
	Context("correct hostname == rescue", func() {
		type testCaseHandleIncompleteBootCorrectHostname struct {
			...
			hostErrorType         infrav2.OperationalState
			expectedReturnError   error
			expectedHostErrorType infrav2.OperationalState
		}
		DescribeTable("hostName = rescue, varying error type and ssh client response - robot client giving all positive results, no timeouts",
			func(tc testCaseHandleIncompleteBootCorrectHostname) {
				...
				Expect(host.Status.OperationalState).To(Equal(tc.expectedHostErrorType))
			},
			Entry("timeout, no errorType", testCaseHandleIncompleteBootCorrectHostname{
				...
				expectedHostErrorType: infrav2.OperationalStateSSHRebootTriggered,
			}),
			Entry("timeout,ErrorType == OperationalStateSoftwareRebootTriggered", testCaseHandleIncompleteBootCorrectHostname{
				isRebootIntoRescue:    true,
				isTimeOut:             true,
				isConnectionRefused:   false,
				hostErrorType:         infrav2.OperationalStateSoftwareRebootTriggered,
				expectedReturnError:   nil,
				expectedHostErrorType: infrav2.OperationalStateSoftwareRebootTriggered,
			}),
			Entry("timeout,ErrorType == OperationalStateHardwareRebootTriggered", testCaseHandleIncompleteBootCorrectHostname{
				isRebootIntoRescue:    true,
				isTimeOut:             true,
				isConnectionRefused:   false,
				hostErrorType:         infrav2.OperationalStateHardwareRebootTriggered,
				expectedReturnError:   nil,
				expectedHostErrorType: infrav2.OperationalStateHardwareRebootTriggered,
			}),
			Entry("timeout,ErrorType == OperationalStateSoftwareRebootTriggered", testCaseHandleIncompleteBootCorrectHostname{
				isRebootIntoRescue:    true,
				isTimeOut:             true,
			...
			Entry("timeout,ErrorType == OperationalStateSSHRebootTriggered", testCaseHandleIncompleteBootCorrectHostname{
				isRebootIntoRescue:    true,
				isTimeOut:             false,
				isConnectionRefused:   false,
				hostErrorType:         infrav2.OperationalStateSSHRebootTriggered,
				expectedReturnError:   nil,
				expectedHostErrorType: infrav2.OperationalStateSoftwareRebootTriggered,
			}),
		...
		DescribeTable("Different reset types",
			...
			Entry("wrong boot, only hw reset, errorType =OperationalStateSSHRebootTriggered", testCaseHandleIncompleteBootDifferentResetTypes{
				isTimeOut:             false,
				isConnectionRefused:   false,
				rebootTypes:           []infrav2.RebootType{infrav2.RebootTypeHardware},
				hostErrorType:         infrav2.OperationalStateSSHRebootTriggered,
			...
			Entry("wrong boot, errorType =OperationalStateSSHRebootTriggered", testCaseHandleIncompleteBootDifferentResetTypes{
				isTimeOut:             false,
				isConnectionRefused:   false,
			...
			Entry("wrong boot,  errorType =OperationalStateSoftwareRebootTriggered", testCaseHandleIncompleteBootDifferentResetTypes{
				isTimeOut:             false,
				isConnectionRefused:   false,
				rebootTypes:           []infrav2.RebootType{infrav2.RebootTypeSoftware, infrav2.RebootTypeHardware},
				hostErrorType:         infrav2.OperationalStateSoftwareRebootTriggered,
			...
			Entry("wrong boot,  errorType =OperationalStateHardwareRebootTriggered", testCaseHandleIncompleteBootDifferentResetTypes{
				isTimeOut:             false,
				isConnectionRefused:   false,
				rebootTypes:           []infrav2.RebootType{infrav2.RebootTypeSoftware, infrav2.RebootTypeHardware},
				hostErrorType:         infrav2.OperationalStateHardwareRebootTriggered,
		...
		It("returns failed if connection error and timed out", func() {
			...
			host := helpers.BareMetalHost("test-host", "default",
				...
				helpers.WithSSHStatus(),
				helpers.WithError(infrav2.OperationalStateConnectionError, ""),
			...
			Expect(host.Status.OperationalState).To(Equal(infrav2.OperationalStateConnectionError))
			Expect(robotMock.AssertNotCalled(GinkgoT(), "RebootBMServer", mock.Anything, mock.Anything)).To(BeTrue())
		})
		...
		It("fails if hardware reboot times out", func() {
			...
			host := helpers.BareMetalHost("test-host", "default",
				...
				helpers.WithSSHStatus(),
				helpers.WithError(infrav2.OperationalStateHardwareRebootTriggered, ""),
				helpers.WithRebootTriggeredAt(metav1.NewTime(time.Now().Add(-time.Hour))),
			)
			service := newTestService(host, &robotMock, nil, nil, nil)
			...
			Expect(host.Status.OperationalState).To(Equal(infrav2.OperationalStateHardwareRebootTriggered))
			Expect(robotMock.AssertNotCalled(GinkgoT(), "RebootBMServer", mock.Anything, mock.Anything)).To(BeTrue())
		})
	})
	...
	Context("hostname rescue vs machinename", func() {
		...
		DescribeTable("vary hostname and see whether rescue gets triggered",
			...
			Entry("hostname == rescue, ErrType == OperationalStateSSHRebootTriggered", testCaseHandleIncompleteBoot{
				isRebootIntoRescue:    true,
				hostErrorType:         infrav2.OperationalStateSSHRebootTriggered,
				expectedReturnError:   nil,
				expectedHostErrorType: infrav2.OperationalStateSoftwareRebootTriggered,
...
var _ = Describe("ensureSSHKey", func() {
	...
	It("sets an error error if a key that exists under another name is uploaded", func() {
		...
		Expect(host.Status.OperationalState).To(Equal(infrav2.OperationalStatePreparationError))
	})
...
var _ = Describe("actionPreparing", func() {
	It("continues when Robot omits rescue availability from the server response", func() {
		...
		Expect(host.Status.OperationalState).To(Equal(infrav2.OperationalStateSoftwareRebootTriggered))
	...
	It("sets a permanent error when the server has no IPv4", func() {
		...
		c := conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition)
		Expect(c).ToNot(BeNil())
		Expect(c.Message).To(ContainSubstring("no IPv4"))
...
var _ = Describe("actionRegistering", func() {
	...
	DescribeTable("actionRegistering",
		func(tc testCaseActionRegistering) {
			...
			if tc.expectedErrorMessage != nil {
				c := conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition)
				Expect(c).ToNot(BeNil())
				Expect(c.Message).To(Equal(*tc.expectedErrorMessage))
			}
	...
	type testCaseActionRegisteringIncompleteBoot struct {
		...
		expectedErrorType infrav2.OperationalState
	}
	...
	DescribeTable("actionRegistering - incomplete reboot",
		func(tc testCaseActionRegisteringIncompleteBoot) {
			...
			if tc.expectedErrorType != infrav2.OperationalState("") {
				Expect(host.Status.OperationalState).To(Equal(tc.expectedErrorType))
			}
		},
		Entry("timeout", testCaseActionRegisteringIncompleteBoot{
			getHostNameOutput: sshclient.Output{Err: timeout},
			expectedErrorType: infrav2.OperationalStateSSHRebootTriggered,
		}),
		Entry("connectionRefused", testCaseActionRegisteringIncompleteBoot{
			...
			expectedErrorType: infrav2.OperationalStateConnectionError,
		}),
	)
})
...
var _ = Describe("actionRegistering check RAID", func() {
	...
	It("check RAID", func() {
		...
		c := conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition)
		Expect(c).ToNot(BeNil())
		Expect(c.Message).Should(Equal("Invalid HetznerBareMetalHost: installImage.swraid of the HetznerBareMetalMachine is active. Use at least two WWNs in spec.rootDevideHints.raid.wwn."))
		...
		c = conditions.Get(host, infrav2.HetznerBareMetalHostActionCompletedCondition)
		Expect(c).ToNot(BeNil())
		Expect(c.Message).Should(Equal("Invalid HetznerBareMetalHost: installImage.swraid of the HetznerBareMetalMachine is not active. Use spec.rootDevideHints.wwn and leave raid.wwn empty."))
	})
})
...
var _ = Describe("actionEnsureProvisioned", func() {
	...
	DescribeTable("actionEnsureProvisioned",
		func(in testCaseActionEnsureProvisioned) {
			...
			if in.expectedErrorType != infrav2.OperationalState("") {
				Expect(host.Status.OperationalState).To(Equal(in.expectedErrorType))
			}
		...
		Entry("correct hostname, cloud init error",
			testCaseActionEnsureProvisioned{
				...
				expectedErrorType:                      infrav2.OperationalStateFatalError,
				expectsSSHClientCallCloudInitStatus:    true,
				expectsSSHClientCallCheckSigterm:       false,
				expectsSSHClientCallReboot:             false,
				expectsOldSSHClientCallCloudInitStatus: false,
