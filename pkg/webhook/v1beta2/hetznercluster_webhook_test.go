...
func TestValidateLoadBalancerHealthCheck(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			healthCheck: &infrav2.LoadBalancerHealthCheckSpec{Protocol: "tcp", IntervalSeconds: ptr.To(5)},
		},
		{
			name:        "http with a path is allowed",
			healthCheck: &infrav2.LoadBalancerHealthCheckSpec{Protocol: "http", Path: ptr.To("/readyz")},
		},
		{
			name:        "https with a domain and a response is allowed",
			healthCheck: &infrav2.LoadBalancerHealthCheckSpec{Protocol: "https", Domain: ptr.To("example.com"), Response: ptr.To("ok")},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cluster := validHetznerCluster(infrav2.LoadBalancerSpec{HealthCheck: tt.healthCheck})

			_, err := webhook.ValidateUpdate(context.Background(), validHetznerCluster(infrav2.LoadBalancerSpec{}), cluster)
