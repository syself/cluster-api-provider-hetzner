...
func TestValidateLoadBalancerHealthCheck(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "tcp health check without path/domain is allowed", healthCheck: &infrav2.LoadBalancerHealthCheckSpec{Type: "tcp"}},
		{name: "http health check with path is allowed", healthCheck: &infrav2.LoadBalancerHealthCheckSpec{Type: "http", Path: ptr.To("/readyz")}},
		{name: "https health check with domain is allowed", healthCheck: &infrav2.LoadBalancerHealthCheckSpec{Type: "https", Domain: ptr.To("example.com")}},
		{
			name:        "tcp health check with path is rejected",
			healthCheck: &infrav2.LoadBalancerHealthCheckSpec{Type: "tcp", Path: ptr.To("/readyz")},
			expectError: true,
		},
		{
			name:        "tcp health check with domain is rejected",
			healthCheck: &infrav2.LoadBalancerHealthCheckSpec{Type: "tcp", Domain: ptr.To("example.com")},
			expectError: true,
		},
		{
