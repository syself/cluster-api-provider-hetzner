...
## Only build
packer build templates/node-image/1.30.10-ubuntu-24-04-containerd/image.json

## Debug and ability to ssh into the created server
packer build --debug --on-error=abort templates/node-image/1.30.10-ubuntu-24-04-containerd/image.json
```
...
Please have a look at the image.json of the [example node-image](https://github.com/syself/cluster-api-provider-hetzner/blob/main/templates/node-image/1.30.10-ubuntu-24-04-containerd/image.json).
