...
| `template.spec.useMatchingIPType`                                | `bool`                | `false`                   | no       | If true, the CIDR suffix is stripped from the machine's NIC IP, and the address is reported as `ExternalIP` (public IP) or `InternalIP` (private IP) in `Machine.status.addresses`, instead of always `InternalIP` with the CIDR suffix kept. Enabling this can affect firewall rules or other apps (e.g. Cilium) that treat `InternalIP` and `ExternalIP` differently. |
