...
| `template.spec.strategy.onExhaustion` | `string` |          | no       | What to do when retries run out and the node is still unhealthy. `Reuse` deletes the machine and frees the host to be provisioned again. Note: when unset, behaves like `Reuse`. `Retire` sets a permanent error on the host, which deletes the machine and keeps the host out of the pool until a human takes the host out of maintenance mode. |
